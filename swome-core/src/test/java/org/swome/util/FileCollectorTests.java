...
public class FileCollectorTests {
	...
	public static void setUp() throws IOException {
		...
		_postfix = ".groovy";
		for (int index = 0; index < MAX_FILES_GROOVY; index++) {
			folder.newFile("SUBFOLDER-B/SUBSUB-B/" + _preffix + index
					+ _postfix);
		}
	...
	public void testCollectWithoutFilters() {
		...
		assertTrue(_fileCollector.collect().size() == MAX_FILES_JAVA
				+ MAX_FILES_GROOVY + MAX_FILES_SCALA);
	}
	...
	public void testCollectionWithRegex() {
		...
		assertTrue(_fileCollector.collect(".*\\.(java|groovy)").size() == MAX_FILES_JAVA
				+ MAX_FILES_GROOVY);
	}
}
