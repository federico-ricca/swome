...
public class GroovyClassDependencyGraphBuilder
		implements
	...
	public Graph<GroovyClassArtefact, ClassReferenceRelation> buildFrom(
			...
			ArtefactFactory<GroovyClassArtefact, GroovyFileArtefact> _factory,
			GraphFactory<GroovyClassArtefact, ClassReferenceRelation> _graphFactory) {
		Graph<GroovyClassArtefact, ClassReferenceRelation> _graph = _graphFactory
				.createGraph();

		for (GroovyFileArtefact _source : _sources) {
			try {
				GroovyClassArtefact _classArtefact = _factory
						.createArtefact(_source);
		...
		for (GroovyClassArtefact _artefact : _graph.artefacts()) {
			...
			if (_superClassId != null) {
				GroovyClassArtefact _superClassArtefact = _graph
						.getArtefact(_superClassId);

				if (_superClassArtefact != null) {
					_graph.addDirectedRelation(
							new SuperclassClassReferenceRelation(), _artefact,
							_superClassArtefact);
				}
			}

			Collection<MethodCallReference> _methodCalls = _artefact
					.getMethodCallReferences();

			for (MethodCallReference _ref : _methodCalls) {
				GroovyClassArtefact _dest = _graph.getArtefact(_ref
						.getTargetClassId());

				if (_dest != null) {
					_graph.addDirectedRelation(
							new MethodCallRelation(_ref.getTargetMethod()),
							_artefact, _dest);
				}
			}
		}
