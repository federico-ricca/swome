...
public class GroovyClassDependencyGraph implements
		Graph<GroovyClassArtefact, ClassReferenceRelation> {
	private Graph<GroovyClassArtefact, ClassReferenceRelation> graph;
	private HashMap<String, GroovyClassArtefact> artefactMap;

	public GroovyClassDependencyGraph() {
		graph = new DefaultDirectedGraph<GroovyClassArtefact, DefaultEdge>(DefaultEdge.class);
	...
	@Override
	public void addDirectedRelation(ClassReferenceRelation _relation,
			GroovyClassArtefact _source, GroovyClassArtefact _dest) {
		graph.addDirectedRelation(_relation, _source, _dest);
	}

	@Override
	public void addRelation(ClassReferenceRelation _relation,
			GroovyClassArtefact _source, GroovyClassArtefact _dest) {
		graph.addRelation(_relation, _source, _dest);
	}

	@Override
	public void addArtefact(GroovyClassArtefact _artefact) {
		graph.addArtefact(_artefact);
	...
	public Collection<RelationArtefactPair<ClassReferenceRelation, GroovyClassArtefact>> getRelationsFor(
		...
		Collection<ClassReferenceRelation> _edges = graph.get
		...
		for (ClassReferenceRelation _edge : _edges) {
			...
			if (_targetArtefact.equals(_pair.getFirst())) {
				_relations
						.add(new RelationArtefactPair<ClassReferenceRelation, GroovyClassArtefact>(
								_edge, _pair.getSecond()));
			} /*
